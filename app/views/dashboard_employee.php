                    ...
                    <div class="report-table-container">
                        <table class="table table-sm table-bordered report-table">
                            <thead id="myReportHead">
                                <tr>
                                    <th class="text-center text-muted">Select date range and click Load Report</th>
                                </tr>
                            </thead>
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header bg-white d-flex justify-content-between align-items-center">
                    ...
                    <button class="btn btn-sm btn-primary" onclick="loadUpcomingSessions(true)">
...
<style>
    ...
    .session-card .badge-time {
        background: #e9f2ff;
        color: #0d47a1;
    }

    .session-card .badge-date {
        background: #f6f9ff;
        color: #0d6efd;
    }
...
<script>
    ...
    async function saveBatchAttendance() {
        ...
        try {
            const res = await fetch('api/attendance.php?action=mark_students', {
                method: 'POST',
                body: form
            });
    ...
    // Load sessions: always load today; future sessions only when includeFuture=true (refresh button)
    async function loadUpcomingSessions(includeFuture = false) {
        const container = document.getElementById('upcomingSessionsContainer');
        if (!container) return;

        // Ensure layout placeholders exist so we can update sections independently
        if (!container.dataset.layout) {
            container.innerHTML = `
                <div id="todaySessionsBlock">
                    <div class="section-title">Today</div>
                    <div id="todaySessionsList" class="mt-1"></div>
                </div>
                <div class="divider"></div>
                <div id="upcomingSessionsBlock">
                    <div class="section-title">Upcoming</div>
                    <div id="upcomingSessionsList" class="mt-1"></div>
                </div>
            `;
            container.dataset.layout = 'ready';
        }

        const todayTarget = document.getElementById('todaySessionsList');
        const upcomingTarget = document.getElementById('upcomingSessionsList');

        try {
            const res = await CRUD.get('api/session_completion.php?action=occurrences&scope=upcoming');
            if (!res.success || !Array.isArray(res.data)) {
                if (todayTarget && todayTarget.innerHTML.trim() === '') {
                    todayTarget.innerHTML = '<p class="text-muted">No sessions</p>';
                }
                if (upcomingTarget && includeFuture) {
                    upcomingTarget.innerHTML = '<p class="text-muted">No upcoming sessions</p>';
                }
                return;
            }
            ...
            const futureSessions = includeFuture ? sessions.filter(s => s.session_date > todayStr) : [];

            // Helper to normalize time (handles "8:00" vs "08:00:00")
            const toMinutes = (t) => {
                if (!t) return 24 * 60; // push undefined to end
            ...
            // Sort today sessions by start_time (chronologically)
            todaySessions.sort((a, b) => toMinutes(a.start_time) - toMinutes(b.start_time));

            // Sort future sessions by date then start_time
            ...
            // Update today section only on initial/default calls (includeFuture === false)
            if (todayTarget && (!todayTarget.dataset.loaded || !includeFuture)) {
                todayTarget.innerHTML = renderCards(todaySessions);
                todayTarget.dataset.loaded = '1';
            }

            // Update upcoming section only when explicitly requested
            if (upcomingTarget) {
                if (includeFuture) {
                    upcomingTarget.innerHTML = renderCards(futureSessions);
                } else if (!upcomingTarget.dataset.placeholder) {
                    upcomingTarget.innerHTML = '<p class="text-muted">Click Refresh to load upcoming sessions.</p>';
                    upcomingTarget.dataset.placeholder = '1';
                }
            }
        } catch (err) {
            console.error('Failed to load sessions', err);
            if (includeFuture && upcomingTarget) {
                upcomingTarget.innerHTML = '<p class="text-danger">Failed to load upcoming sessions</p>';
            } else if (todayTarget && todayTarget.innerHTML.trim() === '') {
                todayTarget.innerHTML = '<p class="text-danger">Failed to load sessions</p>';
            }
        }
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        loadUpcomingSessions(false); // load only today by default
        resetNotesFields();
    });
</script>
...
<script>
    ...
    async function submitSessionCompletion(evt) {
        ...
        try {
            let res = await fetch('api/session_completion.php?action=create', {
                method: 'POST',
                body: data
            });
            ...
            if (status === 'completed') {
                ...
                res = await fetch('api/session_completion.php?action=complete', {
                    method: 'POST',
                    body: compForm
                });
                json = await res.json();
                ...
                res = await fetch('api/session_completion.php?action=cancel', {
                    method: 'POST',
                    body: cancelForm
                });
                json = await res.json();
            ...
            if (noteValues.length > 0) {
                for (const noteText of noteValues) {
                    ...
                    try {
                        await fetch('api/session_completion.php?action=add_note', {
                            method: 'POST',
                            body: nf
                        });
                    } catch (e) {}
